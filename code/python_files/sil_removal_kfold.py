...
# train data
#hist = model1.fit(voiced_feat, Y, batch_size=32, epochs=50, verbose=1, shuffle=True,
...
# using kfold, current running epoch=30, batch_size=16
estimator = KerasClassifier(build_fn=attention_model, epochs=50, batch_size=32, verbose=0)
kfold = KFold(n_splits=10, shuffle=True, random_state=seed)

results = cross_val_score(estimator, voiced_feat, dummy_y, cv=kfold)
print("Mean: %.2f%%, Max: %.2f%%, Min: %.2f%%, STD: %.2f%%" % (results.mean()*100, results.max()*100, result.min()*100, results.std()*100))
